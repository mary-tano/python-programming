...
while running :
  ...
  # Проверка и вывод времени
  Time = Game.getTime(False)
  Game.showAll(0, Blue)

  # Когда время истекло, игра заканчивается
  if Time > bugMax*1500 :
    Game.showMessage("Game Over", Red)
    running = False
  ...
  Window.fill(Green)
  Window.blit(Game.Text, (xMax/4, 10))
  for Nr in range(0,bugMax) :
    Window.blit(Figure[Nr].Bild, (Figure[Nr].x, Figure[Nr].y))
  pg.display.update()

# Pygame после приостановки
pg.time.delay(1500)
pg.quit()
